 ...
 *   /tmp/ri/build/panelgeo.o /tmp/ri/build/ctlreg.o /tmp/ri/build/knob_art.o -lpng \
            ...
            { /* one PART line per (section, size): duplicates fail the parse */
                ...
                if (!dup) {
                    if (nemitted >= 32) {
                        ...
                        nemitted++;
                        fprintf(mf, "PART.%u.0.knob.frame.%dx%d=%s,%d\n", s,
                                w2, h2, sizes[sizeidx].name, FRAMES);
                    }
                }
            }
        }
