...
int ri_sui_init(struct RISectUI *s, uint8_t section) {
    ...
    if (ispat(s))
        return ri_spat_init(&s->u.pat, section);
    if (istr(s))
        return ri_str_init(&s->u.tr);
    if (ismix(s)) {
...
int ri_sui_value(const struct RISectUI *s, uint32_t idx) {
    ...
    if (ispat(s))
        return ri_spat_value(&s->u.pat, idx);
    if (istr(s))
        return ri_str_value(&s->u.tr, idx);
    return 0;
}
...
int ri_sui_step(struct RISectUI *s, uint32_t idx, int dir) {
    if (!s)
        return 0;
    return isfx(s) ? ri_sfx_step(&s->u.fx, idx, dir) : ispat(s) ? ri_spat_step(&s->u.pat, idx, dir)
         : istr(s) ? ri_str_step(&s->u.tr, idx, dir) : 0;
}
